...
{% raw %}
[# iOS](Removed_Local_Path)

If you want to optimize your content for iOS, then it is beneficial for you to [learn more about iOS hardware devices](Removed_Local_Path).
...
Use iOS native [PVRT compression formats](Removed_Local_Path). They will not only decrease the size of your textures (resulting in faster load times and smaller memory footprint), but also can dramatically increase your rendering performance! Compressed texture requires only a fraction of memory bandwidth compared to full blown 32bit RGBA textures. For performance comparison check [iOS Hardware Guide](Removed_Local_Path).
...
Bake your scene static lighting into textures using Unity built-in [Lightmapper](Removed_Local_Path). The process of generating a lightmapped environment takes only a little longer than just placing a light in the scene in Unity, **but**:
...
* [Optimizing iOS Performance](Removed_Local_Path)
* [iOS Hardware Guide](Removed_Local_Path)
* [iOS Automatic Draw Call Batching](Removed_Local_Path)
* [Modeling Optimized Characters](Removed_Local_Path)
* [Rendering Statistics](Removed_Local_Path)
{% endraw %}